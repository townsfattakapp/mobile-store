      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-[#6e6e73] space-y-4">
              ...
              <button onClick={closeCart} className="text-black font-medium underline">Continue Shopping</button>
            </div>
          ) : (
            items.map((item) => (
              <div key={`${item.productId}-${item.variantId}`} className="flex gap-4 border-b pb-6">
                <div className="w-20 h-24 bg-gray-50 rounded-lg flex items-center justify-center p-2 shrink-0 border">
                  ...
                  <img src={item.image} alt={item.name} className="object-contain max-h-full mix-blend-multiply" />
                </div>
        ...
        {items.length > 0 && (
          <div className="p-6 border-t bg-gray-50">
            ...
            <p className="text-xs text-[#6e6e73] mb-6 text-center">Shipping and taxes calculated at checkout.</p>
            
            <Link href="/checkout" onClick={closeCart}>
              <button className="w-full bg-black text-white py-4 rounded-xl font-bold text-lg hover:bg-gray-900 transition-colors shadow-lg shadow-black/10">
                Proceed to Checkout
              </button>
            </Link>
          </div>
