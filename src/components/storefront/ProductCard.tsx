...
export function ProductCard({ product }: ProductCardProps) {
  // Try to use master reference image, fallback to placeholder with white background
  const image = product.main_image_url || "https://placehold.co/400x500/ffffff/a1a1aa?text=No+Image";
  ...
  return (
    <div className="group bg-white rounded-[28px] border border-gray-100 hover:shadow-xl hover:border-gray-200 transition-all duration-300 overflow-hidden flex flex-col h-full relative">
      ...
      {/* Badges in normal flow to prevent overlap - Positioned absolutely over image */}
      ...
      {/* Image Container (Top) */}
      <div className="relative w-full pt-10 px-6 pb-6 bg-[#fbfbfd] flex items-center justify-center">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img 
          src={image} 
          alt={product.name}
          className="object-contain h-[250px] w-auto group-hover:scale-105 transition-transform duration-700 ease-out drop-shadow-lg"
        />
      </div>
        ...
        <div className="mb-auto">
          {product.master_devices?.specifications && (
            <p className="text-xs font-medium text-[#6e6e73] line-clamp-2 px-2">
              {product.master_devices.specifications.processor || product.master_devices.specifications.display || "Pro cameras. Pro design."}
            </p>
          )}
        </div>
