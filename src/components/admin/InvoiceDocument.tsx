...
export function InvoiceDocument({ invoice }: Props) {
  ...
  return (
    <div className="bg-white text-[#1d1d1f] text-[11px] leading-snug relative">
      {isCancelled && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <span className="text-6xl font-black text-red-500/20 rotate-[-24deg] border-4 border-red-500/30 px-8 py-4 uppercase tracking-widest">
            Cancelled
          </span>
        </div>
      )}
      ...
      <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-4">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#6e6e73] mb-1">
          ...
          {isBillOfSupply && (
            <p className="text-[10px] text-[#6e6e73] mt-1 max-w-md">
              Composition taxable person, not eligible to collect tax on supplies
            </p>
          )}
          {!invoice.is_gst && (
            <p className="text-[10px] text-[#6e6e73] mt-1">Non-GST Invoice / Bill</p>
          )}
        </div>
        <div className="text-right">
          <p className="text-lg font-bold tracking-tight">{store.trade_name || store.legal_name}</p>
          <p className="text-[#424245] mt-1">
            {store.address_line1}
          ...
          {store.gstin && (
            <p className="font-semibold mt-1">GSTIN: {store.gstin}</p>
          )}
      ...
      <div className="grid grid-cols-2 gap-6 mb-5">
        <div className="border border-gray-300 rounded-lg p-3">
          <p className="text-[10px] font-bold uppercase tracking-wider text-[#6e6e73] mb-2">
            Bill To / Consignee
          </p>
          ...
          {customer.mobile_number && <p className="mt-1">Mobile: {customer.mobile_number}</p>}
          {customer.email && <p>Email: {customer.email}</p>}
          {customer.gstin && <p className="font-semibold mt-1">GSTIN: {customer.gstin}</p>}
        </div>
      ...
      <table className="w-full border-collapse mb-4">
        <thead>
          <tr className="bg-neutral-100 border border-gray-300">
            ...
            <th className="border border-gray-300 px-2 py-2 text-left">HSN</th>
          ...
          {items.map((item, idx) => (
            <tr key={idx} className="align-top">
              ...
              <td className="border border-gray-300 px-2 py-2">
                ...
                <p className="text-[#6e6e73] font-mono text-[10px]">SKU: {item.sku}</p>
              </td>
              <td className="border border-gray-300 px-2 py-2">{item.hsn}</td>
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-right">
                    {item.cgst > 0 ? (
                      <>
                        ...
                        <span className="block text-[9px] text-[#6e6e73]">@{item.cgst_rate}%</span>
                      </>
                    ) : (
                      "—"
                    )}
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-right">
                    {item.sgst > 0 ? (
                      <>
                        ...
                        <span className="block text-[9px] text-[#6e6e73]">@{item.sgst_rate}%</span>
                      </>
                    ) : (
                      "—"
                    )}
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-right">
                    {item.igst > 0 ? (
                      <>
                        ...
                        <span className="block text-[9px] text-[#6e6e73]">@{item.igst_rate}%</span>
                      </>
                    ) : (
                      "—"
                    )}
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-[#6e6e73] mb-1">
            Amount in Words
          </p>
          <p className="font-semibold text-sm border border-gray-300 rounded-lg p-3 bg-neutral-50">
            ...
            <tbody>
              {showGstCols && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">Taxable Value</td>
              ...
              {totals.shipping_charge > 0 && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">Shipping / Freight</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              {showGstCols && totals.cgst_total > 0 && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">CGST</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              {showGstCols && totals.sgst_total > 0 && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">SGST</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              {showGstCols && totals.igst_total > 0 && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">IGST</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              {showGstCols && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">Total Tax</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              {totals.round_off != null && Number(totals.round_off) !== 0 && (
                <tr className="border-b border-gray-200">
                  <td className="px-3 py-2 text-[#6e6e73]">Round Off</td>
                  <td className="px-3 py-2 text-right font-medium">
              ...
              <tr className="bg-neutral-100">
                <td className="px-3 py-3 font-bold text-sm">Grand Total</td>
                <td className="px-3 py-3 text-right font-black text-base">
      ...
      <div className="grid grid-cols-2 gap-6 pt-4 border-t border-gray-300">
        ...
        <div className="text-right flex flex-col items-end justify-end min-h-[80px]">
          <p className="font-semibold">For {store.legal_name}</p>
          <div className="h-12" />
          <p className="border-t border-gray-400 pt-1 text-[#6e6e73]">
            {store.authorized_signatory || "Authorized Signatory"}
          </p>
        </div>
      </div>

      <p className="text-center text-[10px] text-[#6e6e73] mt-6">
