  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={fetchBrands} variant="outline" className="flex items-center gap-2">
        ...
        <div className="bg-white p-6 rounded-xl border shadow-sm h-fit">
          ...
          <form onSubmit={handleAddBrand} className="space-y-4">
            ...
            <Button type="submit" className="w-full flex items-center justify-center gap-2" disabled={saving}>
              {saving ? "Saving..." : <><Plus size={18} /> Add Brand</>}
            </Button>
          </form>
        </div>
                  ...
                  brands.map((brand) => (
                    <tr key={brand.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${brand.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
